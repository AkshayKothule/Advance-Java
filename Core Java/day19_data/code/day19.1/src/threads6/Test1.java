...
public class Test1 {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		System.out.println("main waiting for t1,t3,t4 threads to complete exec.");
		System.out.println(t1.isAlive()+" "+t4.isAlive());//t t
		t1.join();
		t3.join();
		t4.join();
		System.out.println("main waiting for 10sec max , for t2");
		t2.join(10000);//main is blocked on joint(t2) - max up to 10 sec
		 //main sends interrupt ---->  t2
		System.out.println("main sending an interrupt -> t2");
		t2.interrupt();
		t2.join();	
