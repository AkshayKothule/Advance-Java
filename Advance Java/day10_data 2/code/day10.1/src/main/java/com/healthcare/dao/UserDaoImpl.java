...
public class UserDaoImpl implements UserDao {
	...
	public List<User> getAllUsers() {
		...
		try {
			...
			tx.commit(); //session.flush ->no dml ->sessoion close -> destroy l1 cache and connection return dbcp
	...
	public List<String> getSelectedUsersLastName(UserRole role1) {
		...
		try {
			...
			//persistent stateb- lastname 
			tx.commit(); //session.flush and session close and connection return dbcp
			// lastname :- detached 
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
	...
	public List<User> getSelectedDetailsByRole(UserRole role1) {
		List<User> users = null;
		//in query give f.q and constructor 
	...
	public String changePassword(String email1, String oldPassword1, String newPass) {
		...
		try {
			user = session.createQuery(jpql, User.class)
					...
					.setParameter("pass", oldPassword1)
					.getSingleResult();
			 ...
			 */
			tx.commit();// session.flush -> DML -> session.close ->l1 cache -> destory 
	...
	public String deleteUserDetailsById(Long userId) {
		...
		// 1. get Session from SessionFactory
				...
				Transaction tx=session.beginTransaction();
				try {
					user=session.find(User.class, userId); //frist check catche present or not if present continue and not fetch from Db
					if(user != null)
					{
						//user - ?????????
						session.remove(user); //user - mark for removal
						mesg="User details deleted ...";
					}
					tx.commit(); //dirty_cj=hecking : session.flush ->dml -> session.close -> l1 Cache destroy and entity remvove from  DB 
					//user - transient 
				} catch (RuntimeException e) {
					if (tx != null)
						tx.rollback();
